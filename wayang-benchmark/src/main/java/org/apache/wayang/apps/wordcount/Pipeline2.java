...
class ForecastResult2 {
    private final float totalFTEs;
    private final int capacity;

    public ForecastResult2(float totalFTEs, int capacity) {
...
public class Pipeline2 {
    ...
    private static final Logger log = LoggerFactory.getLogger(Pipeline2.class);
    ...
    public static void main(String[] args) {
        ...
        try {
            ForecastResult2 forecastResult = ForecastPipeline(planBuilder, urlForecast);
            // Collection<JSONObject> all_data = HubspotPipeline(planBuilder, urlHubspot);
    ...
    private static ForecastResult2 ForecastPipeline(JavaPlanBuilder planBuilder, String urlForecast) {
        ...
        try {
            ...
            Collection<Tuple2<Float,String>> filteredData = planBuilder
                .readRestAPISource(urlForecast, apiMethod, headers)  // Step 1: Read API source
                .filter(json -> allowedRoles.contains(json.optString("Roles", "")))  // Step 2: Filter data
                .map(json -> {
                    // Step 3: Map to Tuple2<Float, String> where Float = FTEs, String = Person
        ...
        return new ForecastResult2(totalFTEs, capacity);
    }
